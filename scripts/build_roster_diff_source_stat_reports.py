#!/usr/bin/env python3
"""Create source-stat work files from roster-diff possible missing transfers."""
...
from build_sports_reference_roster_diff_audit import profile_cache_path
from scrape_phase1_d1_outcomes import parse_table_rows


INPUT_PATH = Path("data/big_west_roster_diff_possible_missing_transfers.csv")
D1_FOUND_PATH = Path("data/big_west_roster_diff_d1_source_stats_found.csv")
NEEDED_PATH = Path("data/big_west_roster_diff_source_stats_needed.csv")
...
def d1_source_row(candidate: dict[str, str], stat: dict[str, str]) -> dict[str, str]:
    return {
        "first_big_west_season": candidate["first_big_west_season"],
...
def main() -> int:
    if not INPUT_PATH.exists():
        raise FileNotFoundError(f"Missing {INPUT_PATH}; run build_sports_reference_roster_diff_audit.py first")
    ...
    with INPUT_PATH.open(newline="", encoding="utf-8") as file:
        for candidate in csv.DictReader(file):
            ...
            if level == "D1":
                profile_path, status = profile_cache_path(candidate["sports_reference_player_url"], profile_slug(candidate["sports_reference_player_url"]))
                ...
                needed.append(needed_row(candidate, "no source stat row from this label; check earlier playable season before adding"))
            else:
