#!/usr/bin/env python3
"""Create source-stat work files from target-conference roster-diff possible transfers."""
...
from build_target_conference_sports_reference_roster_diff_audit import profile_cache_path
...
def d1_source_row(candidate: dict[str, str], stat: dict[str, str]) -> dict[str, str]:
    return {
        ...
        "position": stat.get("pos", candidate["position"]),
...
def main() -> int:
    ...
    input_path = Path(f"data/{args.conference}_roster_diff_possible_transfers.csv")
    if not input_path.exists():
        raise FileNotFoundError(f"Missing {input_path}; run roster-diff audit first")
        ...
        for candidate in csv.DictReader(file):
            ...
            if level == "D1":
                ...
                if not profile_path or status != "cached":
                    ...
                    continue
                rows = parse_table_rows(profile_path.read_text(encoding="utf-8", errors="ignore"), "players_per_game")
    ...
    d1_path = Path(f"data/{args.conference}_roster_diff_d1_source_stats_found.csv")
    needed_path = Path(f"data/{args.conference}_roster_diff_source_stats_needed.csv")
    summary_path = Path(f"data/{args.conference}_roster_diff_source_stats_summary.csv")
    write_csv(d1_path, D1_COLUMNS, d1_found)
    write_csv(needed_path, NEEDED_COLUMNS, needed)

    summary_counts: dict[tuple[str, str], int] = {}
    for row in d1_found:
        key = ("d1_source_stats_found", row["first_target_season"])
        summary_counts[key] = summary_counts.get(key, 0) + 1
    for row in needed:
        key = (row["needed"], row["first_target_season"])
        summary_counts[key] = summary_counts.get(key, 0) + 1
    summary_rows = [
        {"bucket": bucket, "first_target_season": season, "count": count}
        for (bucket, season), count in sorted(summary_counts.items())
    ]
    write_csv(summary_path, ["bucket", "first_target_season", "count"], summary_rows)

    print(f"Wrote {len(d1_found)} D1 source stat rows to {d1_path}")
